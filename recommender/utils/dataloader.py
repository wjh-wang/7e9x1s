...
class TSDataset(Dataset):
    def __init__(self, config, data, mode = "train"):
        self.batch_size = config["TS_batch_size"]
        self.mode = mode
        if self.mode=="train":
            self.train = True
        else:
    ...
    def __getitem__(self, idx):
        ...
        pattern = torch.tensor(self.pattern[idx], dtype=torch.int32)
        if self.train:
            label = torch.tensor(self.labels[idx], dtype=torch.float32)
            return feature, pattern, label
        else:
...
class TrainDataLoader(AbstractDataLoader):
    ...
    def __init__(self, config, dataset, batch_size=1, shuffle=False):
        ...
        self.all_uids = self.dataset.df[self.dataset.uid_field].unique().tolist()
    ...
    def _get_full_uids_sample(self):
        # user_ids = self.all_uids[self.pr: self.pr + self.step]
        user_ids = random.sample(self.all_uids, self.step)
        # if self.batch_size <= self.n_users:
        #     user_ids = random.sample(self.all_uids, self.batch_size)
        # else:
        #     user_ids = [random.choice(self.all_uids) for _ in range(self.batch_size)]
            
        def sample_pos_items_for_u(u, num):
            pos_items = list(self.history_items_per_u[u])
        ...
        def sample_neg_items_for_u(u, num):
            ...
            while True:
                ...
                neg_id = np.random.randint(low=0, high=self.all_item_len, size=1)[0]
                if neg_id not in self.history_items_per_u[u] and neg_id not in neg_items:
        ...
        # pos_items = sample_pos_items_for_u(user_ids,1)
        # neg_items = sample_neg_items_for_u(user_ids,1)
        ...
        pos_i_tensor = torch.tensor(pos_items).type(torch.LongTensor).to(self.device)
        neg_i_tensor = torch.tensor(neg_items).type(torch.LongTensor).to(self.device)
        ...
        # print("xx: ",len(self.all_uids))
        # return user_tensor
        return torch.cat([user_tensor.unsqueeze(0), pos_i_tensor.unsqueeze(0), neg_i_tensor.unsqueeze(0)],dim=0)
